...
Linear uses [Vitest](https://vitest.dev/) for unit and integration tests. Tests enforce the layered architecture by mocking one layer deep, keeping each layer testable in isolation.
...
npm run test:commands   # Executed CLI command coverage gate
```
...
The command coverage gate (`npm run test:commands`) runs command-unit tests and
the tokenless integration suite with runtime instrumentation. It credits only
Commander parses and CLI subprocess launches that actually execute; skipped
live-API tests and unreachable source do not count. The report compares those
paths with the authoritative Commander tree and lists the remaining gaps.

## Integration Tests

Integration tests execute the compiled CLI binary and validate its output.
Help and local-validation cases run without a token; live API cases require a
real Linear API token and are skipped when it is absent.
