...
Linear is a CLI tool for [Linear.app](https://linear.app) that outputs structured JSON. It uses a layered architecture with strict TypeScript, GraphQL code generation, and ES modules.
...
Linear uses [GraphQL Code Generator](https://the-guild.dev/graphql/codegen) to produce typed query documents and result types. Never write raw GraphQL strings in TypeScript.
...
For interactive setup, run `linear auth login` — it opens Linear in the browser and stores the token encrypted in `~/linear/token`.

The API token is resolved in this order:

1. `--api-token <token>` command-line flag
2. `LINEAR_API_TOKEN` environment variable
3. `~/linear/token` (encrypted, set up via `linear auth login`)
...
| `npm run test:commands` | Enforce executed CLI command-path coverage |
