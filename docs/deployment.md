...
Linear is a CLI tool for Linear.app that compiles from TypeScript to JavaScript during installation. It runs on Node.js 22+ and outputs JSON for all commands.
...
This creates the `linear` command, pointing to `dist/main.js`.

### Direct Git Install

```bash
npm install git+https://github.com/HumanInTheLoopReal/linear.git
```
...
For interactive use (humans), run `linear auth login` — it opens Linear in the browser and stores the token encrypted in `~/linear/token`.

Linear checks for an API token in this order:
...
docker run -e LINEAR_API_TOKEN=lin_api_... linear issue list
```
