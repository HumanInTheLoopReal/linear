...
Linear is a command-line interface for [Linear.app](https://linear.app) that outputs structured JSON. It is built for automation, scripting, and integration with other tools, including LLM agents.
...
Interactive setup (for humans): `linear auth login` — opens Linear in the browser and stores the token encrypted in `~/linear/token`.
