...
describe("resolveSearchFilterIds", () => {
  ...
  it("maps logical status aliases to stateTypes without hitting resolveStatusId (lin-zst6)", async () => {
    const sdk = {} as unknown as LinearSdkClient;
    ...
    expect(result.stateIds).toBeUndefined();
    expect(result.stateTypes).toEqual(["triage", "backlog", "unstarted"]);
  });

  it("unions stateTypes across multiple logical aliases (lin-zst6)", async () => {
    const sdk = {} as unknown as LinearSdkClient;
  ...
  it("rejects mixing logical aliases with team-specific names (lin-zst6)", async () => {
    ...
    await expect(
      resolveSearchFilterIds(sdk, {
        team: "ENG",
        statusNames: ["open", "Backlog"],
      }),
