...
describe("resolveIssueTeamContext", () => {
  it("returns UUIDs from the issue teamId projection", async () => {
    const client = mockSdkClient([{ id: "issue-uuid", teamId }]);

    await expect(resolveIssueTeamContext(client, "ENG-42")).resolves.toEqual({
      issueId: "issue-uuid",
      teamId,
    });
  ...
  it("resolves a team key when the issue relation has no id", async () => {
    const client = mockSdkClient(
      [{ id: "issue-uuid", team: { key: "ENG" } }],
      [exponentialTeam],
    );
  ...
  it("throws when the issue is missing", async () => {
    ...
    await expect(resolveIssueTeamContext(client, "ENG-404")).rejects.toThrow(
