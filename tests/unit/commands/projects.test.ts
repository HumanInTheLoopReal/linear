...
describe("projects discussion commands", () => {
  ...
  it("projects discuss resolves project and delegates to discussion service", async () => {
    ...
    expect(outputResult).toHaveBeenCalledWith(
      { id: "discussion-root-1" },
      expect.any(Function),
      expect.anything(),
    );
    expect(checkCommentFormat).toHaveBeenCalledWith("Kickoff thread");
  });

  it("projects discuss rejects whitespace-only bodies", async () => {
    const program = createProgram();
  ...
  it("blocks every project discussion write when formatting is rejected", async () => {
    const commands = [
      ["projects", "discuss", "My Project", "--body", "wall"],
      ["projects", "reply", "thread-1", "--body", "wall"],
      ["projects", "edit", "comment-1", "--body", "wall"],
      ["projects", "edit-reply", "reply-1", "--body", "wall"],
    ];

    for (const argv of commands) {
      vi.mocked(checkCommentFormat).mockImplementationOnce(() => {
        throw new Error("comment formatting rejected");
      });
      await createProgram().parseAsync(["node", "test", ...argv]);
    }
    ...
    expect(checkCommentFormat).toHaveBeenCalledTimes(commands.length);
  });
  ...
  it("projects delete-comment delegates to generic discussion comment service", async () => {
    ...
    expect(outputResult).toHaveBeenCalledWith(
      { id: "discussion-comment-1", success: true },
      expect.any(Function),
      expect.anything(),
    );
  });
  ...
  it("projects delete-reply delegates to discussion service", async () => {
    ...
    expect(outputResult).toHaveBeenCalledWith(
      { id: "discussion-reply-1", success: true },
      expect.any(Function),
      expect.anything(),
    );
  });
