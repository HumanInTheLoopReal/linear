...
describe("discussion resolution formatters", () => {
  it("renders the resolving user when available", () => {
    expect(
      formatThreadResolved({
        id: "thread-1",
        resolvingUser: { id: "user-1", displayName: "Ada" },
      }),
    ).toBe("✓ Thread resolved by Ada (id: thread-1)\n");
  });

  it("renders unresolved acknowledgements", () => {
    expect(formatThreadUnresolved({ id: "thread-1" })).toBe(
      "✓ Thread unresolved (id: thread-1)\n",
    );
  });
});
...
describe("comments compatibility delegation", () => {
  ...
  it("comments create rejects whitespace-only bodies", async () => {
    const program = createProgram();
