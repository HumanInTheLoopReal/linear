...
describe("initiative discussion commands", () => {
  ...
  it("wires reply", async () => {
    ...
    expect(checkCommentFormat).toHaveBeenCalledWith("Nested reply");
  });
  ...
  it("wires generic edit to discussion comment service", async () => {
    ...
    expect(checkCommentFormat).toHaveBeenCalledWith("Edited");
  });
  ...
  it("blocks every initiative discussion write when formatting is rejected", async () => {
    const commands = [
      ["initiatives", "discuss", "Growth", "--body", "wall"],
      ["initiatives", "reply", "thread-1", "--body", "wall"],
      ["initiatives", "edit", "comment-1", "--body", "wall"],
      ["initiatives", "edit-reply", "reply-1", "--body", "wall"],
    ];
