...
describe("getApiToken", () => {
  ...
  it("throws a missing-kind AuthenticationError when no token found anywhere", () => {
    vi.mocked(getStoredToken).mockReturnValue(null);
    vi.mocked(fs.existsSync).mockReturnValue(false);

    expect(() => getApiToken({})).toThrow(AuthenticationError);
    ...
    const authErr = thrown as AuthenticationError;
    expect(authErr.kind).toBe("missing");
    expect(authErr.message).toBe("No API token found.");
    expect(authErr.remediation.join("\n")).toContain("LINEAR_API_TOKEN");
  });
});
