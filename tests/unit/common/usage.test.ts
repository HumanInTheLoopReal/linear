...
describe("formatOverview", () => {
  it("formats overview with version, auth, and all domain summaries", () => {
    ...
    expect(result).toContain("linear v2025.12.3");
    ...
    expect(result).toContain("detail: linear <domain> usage");
  });
...
describe("formatDomainUsage", () => {
  it("renders a flat command's own signature, description, and options", () => {
    const command = new Command("assign")
      .description("assign an issue to a user")
      .argument("<issue>")
      .argument("<user>")
      .option("--force", "replace the current assignee");
    const meta: DomainMeta = {
      name: "assign",
      summary: "assign work",
      context: "assignment is a single operation.",
      arguments: {
    ...
    const result = formatDomainUsage(command, meta);

    expect(result).toContain("commands:");
    expect(result).toContain("assign <issue> <user>");
    expect(result).toContain("assign an issue to a user");
    expect(result).toContain("assign options:");
  ...
  it("formats domain with commands, arguments, options, and see-also", () => {
    ...
    // Header
    expect(result).toContain(
      "linear issues — work items with status, priority, assignee, labels",
    );
    // Context
  ...
  it("omits arguments and see-also sections when empty", () => {
    ...
    expect(result).toContain("linear teams — organizational units");
