...
vi.mock("@linear/sdk", () => {
  ...
  const mockConstructorCalls: Array<{
    apiKey?: string;
    apiUrl?: string;
    signal?: AbortSignal;
  }> = [];
...
const QUERY_DOCUMENT = parse("query RetryTest { viewer { id } }");
const MUTATION_DOCUMENT = parse(
  'mutation RetryTest { issueCreate(input: { title: "test", teamId: "team" }) { success } }',
);
...
describe("GraphQLClient", () => {
  ...
  describe("request", () => {
    ...
    let mockConstructorCalls: Array<{
      apiKey?: string;
      apiUrl?: string;
      signal?: AbortSignal;
    }>;
    ...
    it("passes a configured endpoint to the Linear client", async () => {
      ...
      await client.request(QUERY_DOCUMENT);

      expect(mockConstructorCalls[0]).toMatchObject({
        apiKey: "test-token",
        apiUrl: "https://proxy.example/graphql",
      });
    });

    it("preserves the SDK default when no endpoint is configured", async () => {
      mockRawRequest.mockResolvedValueOnce({ data: { ok: true } });
      const client = new GraphQLClient("test-token");

      await client.request(QUERY_DOCUMENT);

      expect(mockConstructorCalls[0]).not.toHaveProperty("apiUrl");
    });
    ...
    it("retries on 429 and succeeds on next attempt", async () => {
      ...
      vi.useFakeTimers();
      try {
        const promise = client.request(QUERY_DOCUMENT);
        await vi.runAllTimersAsync();
    ...
    it("does not retry a mutation after a retryable transport error", async () => {
      mockRawRequest.mockRejectedValue({ response: { status: 503 } });
      const client = new GraphQLClient("good-token");

      await expect(client.request(MUTATION_DOCUMENT)).rejects.toThrow(
