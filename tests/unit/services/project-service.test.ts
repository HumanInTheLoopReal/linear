...
describe("listProjects — chunked pagination under complexity cap (lin-n7hh)", () => {
  // A response page of `count` placeholder projects + pageInfo.
  function page(count: number, hasNextPage: boolean, endCursor: string | null) {
    return {
      projects: {
  ...
  // A client whose request() returns each queued page in order.
  function sequencedClient(pages: ReturnType<typeof page>[]): GraphQLClient {
    const request = vi.fn();
    for (const p of pages) request.mockResolvedValueOnce(p);
    return { request } as unknown as GraphQLClient;
  }

  it("never asks for more than the safe page size, even at limit 100", async () => {
    ...
    // The defect: a single first:100 request costs ~13950 > 10000 and errors.
    // The fix splits it into two safe 50-node requests.
    ...
    // Final pageInfo comes from the last fetched page.
    expect(result.pageInfo).toEqual({ hasNextPage: false, endCursor: "c100" });
  });

  it("stops early when the server runs out of pages", async () => {
    const client = sequencedClient([page(30, false, "c30")]);
    const result = await listProjects(client, { limit: 100 });
    expect(client.request).toHaveBeenCalledTimes(1);
    expect(result.nodes).toHaveLength(30);
  });

  it("slices to exactly the requested limit when a page overshoots", async () => {
    const client = sequencedClient([
      page(50, true, "c50"),
      page(50, true, "c100"),
    ]);
    const result = await listProjects(client, { limit: 75 });
    // page 1: first=50; page 2: first=min(25,50)=25 (server still returns 50)
    expect(client.request).toHaveBeenNthCalledWith(2, expect.anything(), {
      first: 25,
      after: "c50",
    });
    expect(result.nodes).toHaveLength(75);
  });

  it("keeps a single request for the default limit of 50", async () => {
    const client = sequencedClient([page(50, true, "c50")]);
    const result = await listProjects(client);
    expect(client.request).toHaveBeenCalledTimes(1);
