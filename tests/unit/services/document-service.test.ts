...
describe("getDocumentCommentTarget", () => {
  it("resolves a slug ID to the document content ID comments hang off", async () => {
    ...
    const result = await getDocumentCommentTarget(client, "ad9a3d2952b9");
  ...
  it("throws when the document is missing", async () => {
    ...
    await expect(getDocumentCommentTarget(client, "missing")).rejects.toThrow(
      'Document with ID "missing" not found',
    );
  });

  it("throws when the document has no content to comment on", async () => {
    const client = mockGqlClient({
      document: { id: "doc-uuid", title: "Empty", documentContentId: null },
    });
    await expect(getDocumentCommentTarget(client, "doc-uuid")).rejects.toThrow(
      'Document "Empty" has no content to comment on',
    );
  });
});
