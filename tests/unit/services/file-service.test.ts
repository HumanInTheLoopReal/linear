...
describe("uploadFile", () => {
  ...
  it("uses a configured GraphQL endpoint for upload authorization", async () => {
    mockSuccessfulUpload();
    const endpoint = "https://proxy.example/graphql";
    const service = new FileService(TEST_TOKEN, endpoint);

    await service.uploadFile("/path/to/image.png");
