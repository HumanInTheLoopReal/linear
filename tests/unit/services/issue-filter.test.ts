...
describe("buildIssueFilter", () => {
  ...
  it("builds state-type include filter", () => {
    const result = buildIssueFilter({ stateTypes: ["unstarted", "started"] });
    expect(result).toEqual({
      and: [{ state: { type: { in: ["unstarted", "started"] } } }],
    });
  });

  it("builds state-type exclude filter", () => {
    const result = buildIssueFilter({
      stateTypesExclude: ["completed", "canceled"],
    });
    expect(result).toEqual({
