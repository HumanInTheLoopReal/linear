...
describe("normalizeTeamKey", () => {
  it("uppercases the value and strips trailing hyphens", () => {
    expect(normalizeTeamKey("kw-")).toMatchObject({ ok: true, key: "KW" });
    expect(normalizeTeamKey("eng")).toMatchObject({ ok: true, key: "ENG" });
    expect(normalizeTeamKey("Mobile")).toMatchObject({
      ok: true,
      key: "MOBILE",
    });
  });

  it("rejects empty / oversized / non-alphanumeric / leading-digit keys", () => {
    expect(normalizeTeamKey("").ok).toBe(false);
    expect(normalizeTeamKey("toolongkey").ok).toBe(false);
    expect(normalizeTeamKey("1eng").ok).toBe(false);
    expect(normalizeTeamKey("with space").ok).toBe(false);
    expect(normalizeTeamKey("a!b").ok).toBe(false);
  });
});

describe("renameTeamKey", () => {
  it("dry-run skips the API call and returns changed=false with the normalized new key", async () => {
    const request = vi.fn();
    const client = { request } as unknown as GraphQLClient;
    const result = await renameTeamKey(client, {
      teamId: "t1",
      currentKey: "OLD",
      newKey: "new-",
      dryRun: true,
    ...
    expect(result.warning).toMatch(/immutable/);
  });

  it("no-ops (no API call) when newKey already equals currentKey", async () => {
    const request = vi.fn();
    const client = { request } as unknown as GraphQLClient;
    const result = await renameTeamKey(client, {
      teamId: "t1",
  ...
  it("calls teamUpdate with the normalized key when run live", async () => {
    const request = vi.fn().mockResolvedValueOnce({
      teamUpdate: {
        success: true,
        team: { id: "t1", key: "NEW", name: "Renamed" },
      },
    });
    const client = { request } as unknown as GraphQLClient;
    const result = await renameTeamKey(client, {
    ...
    expect(vars.input.key).toBe("NEW");
    expect(result).toMatchObject({
      team_id: "t1",
      old_key: "OLD",
      new_key: "NEW",
  ...
  it("rejects an invalid key with a descriptive error", async () => {
    const request = vi.fn();
    const client = { request } as unknown as GraphQLClient;
    await expect(
      renameTeamKey(client, {
        teamId: "t1",
        currentKey: "ENG",
        newKey: "1bad",
      }),
    ).rejects.toThrow(/invalid team key/);
    expect(request).not.toHaveBeenCalled();
  });

  it("surfaces teamUpdate failures as throws", async () => {
    const request = vi.fn().mockResolvedValueOnce({
    ...
    ).rejects.toThrow(/teamUpdate failed/);
  });
});
