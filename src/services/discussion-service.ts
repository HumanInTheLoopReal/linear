...
import {
  ...
  ListDocumentDiscussionReplyCandidatesDocument,
  type ListDocumentDiscussionReplyCandidatesQuery,
  ListDocumentDiscussionReplyCandidatesWithReactionsDocument,
  type ListDocumentDiscussionReplyCandidatesWithReactionsQuery,
  ListDocumentDiscussionRootsDocument,
  type ListDocumentDiscussionRootsQuery,
  ListDocumentDiscussionRootsWithReactionsDocument,
  type ListDocumentDiscussionRootsWithReactionsQuery,
...
export type DiscussionEntityKind =
  | "issue"
  | "project"
  | "initiative"
  | "document";
...
type DiscussionReplyCandidateQuery =
  ...
  | ListDocumentDiscussionReplyCandidatesQuery;
...
type DiscussionReplyCandidateWithReactionsQuery =
  ...
  | ListDocumentDiscussionReplyCandidatesWithReactionsQuery;
...
function getDiscussionEntityKind(
  comment: Pick<
    ...
    "issueId" | "projectId" | "initiativeId" | "documentContentId"
  >,
...
/**
 * The ID a thread's replies are queried by.
 *
 * For documents this is the `documentContentId`, not the document ID: Linear
 * anchors comments to the document's DocumentContent entity, so that is what
 * the comment filter matches on.
 */
function getDiscussionThreadEntity(
  ...
  if (thread.documentContentId) {
    return { kind: "document", id: thread.documentContentId };
  }
...
async function listDiscussionReplyCandidates(
  ...
  while (true) {
    ...
    if (entity.kind === "issue") {
      ...
      result = await client.request<ListDocumentDiscussionReplyCandidatesQuery>(
        ListDocumentDiscussionReplyCandidatesDocument,
        {
          documentContentId: entity.id,
          first: DISCUSSION_REPLY_FETCH_LIMIT,
          after,
        },
      );
...
async function listDiscussionReplyCandidatesWithReactions(
  ...
  while (true) {
    ...
    if (entity.kind === "issue") {
      ...
      result =
        await client.request<ListDocumentDiscussionReplyCandidatesWithReactionsQuery>(
          ListDocumentDiscussionReplyCandidatesWithReactionsDocument,
          {
            documentContentId: entity.id,
            first: DISCUSSION_REPLY_FETCH_LIMIT,
...
/**
 * `documentId` accepts a slug ID or a UUID — `document(id:)` resolves both,
 * matching `documents read`.
 */
export async function listDiscussionsForDocument(
  client: GraphQLClient,
  documentId: string,
  options: PaginationOptions = {},
): Promise<PaginatedResult<DiscussionThread>> {
  const { limit = DEFAULT_ROOT_LIMIT, after } = options;
  const result = await client.request<ListDocumentDiscussionRootsQuery>(
    ListDocumentDiscussionRootsDocument,
    {
      documentId,
      first: limit,
  ...
  return {
    nodes: result.document.comments.nodes,
    pageInfo: result.document.comments.pageInfo,
  };
}

export async function listDiscussionsForDocumentWithReactions(
  client: GraphQLClient,
  documentId: string,
  options: PaginationOptions = {},
): Promise<PaginatedResult<DiscussionThreadWithReactions>> {
  const { limit = DEFAULT_ROOT_LIMIT, after } = options;
  const result =
    await client.request<ListDocumentDiscussionRootsWithReactionsQuery>(
      ListDocumentDiscussionRootsWithReactionsDocument,
      {
        documentId,
        first: limit,
  ...
  return {
    nodes: normalizeDiscussionCommentsReactions(result.document.comments.nodes),
    pageInfo: result.document.comments.pageInfo,
  };
}
...
/**
 * Posts against the document's `documentContentId` — the comment target for
 * documents. Root comments posted this way are unanchored; `quotedText`
 * anchoring is a human, in-editor action (agents edit documents rather than
 * annotate them), so the CLI reads anchors but does not write them.
 */
export async function startDocumentDiscussion(
  client: GraphQLClient,
  input: { documentContentId: string; body: string },
): Promise<StartDiscussionMutation["commentCreate"]["comment"]> {
  return startDiscussion(client, {
